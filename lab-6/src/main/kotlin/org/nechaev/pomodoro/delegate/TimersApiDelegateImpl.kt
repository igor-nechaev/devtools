...
class TimersApiDelegateImpl(
    ...
    private val durationSummary: DistributionSummary =
        DistributionSummary
            .builder("pomodoro.timer.duration")
    ...
    init {
        ...
        meterRegistry.gauge("pomodoro.gauge.completion.rate", timerRepository) { repo ->
            ...
            if (total == 0.0) {
                ...
                repo.countByStatus(TimerStatus.COMPLETED).toDouble() / total
            }
        }
    }
    ...
    override fun startTimer(id: Long): ResponseEntity<Timer> =
        span("pomodoro.start-timer") { obs ->
            ...
            val saved =
                span("pomodoro.state.transition.running") {
    ...
    override fun stopTimer(id: Long): ResponseEntity<Timer> =
        span("pomodoro.stop-timer") { obs ->
            ...
            val saved =
                span("pomodoro.state.transition.paused") { childObs ->
    ...
    override fun completeTimer(id: Long): ResponseEntity<Timer> =
        span("pomodoro.complete-timer") { obs ->
            obs.lowCardinalityKeyValue("timer.id", id.toString())
            val entity = findTimerOrThrow(id)
            ...
            val saved =
                span("pomodoro.state.transition.completed") { childObs ->
                    entity.accumulateElapsed()
    ...
    private fun findTimerOrThrow(id: Long): TimerEntity =
        span("pomodoro.repository.find-by-id") { obs ->
            obs.lowCardinalityKeyValue("timer.id", id.toString())
    ...
    private fun TimerEntity.toDto() =
        autoCompleteIfExpired().let {
            Timer(
                id = it.id,
                name = it.name,
